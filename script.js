...
function loadSample(){
  ...
  const pcs = [
    ...
    ['PC-C',300,460,30,'192.168.30.10','192.168.30.1'],
    ['PC-D',420,460,40,'192.168.40.10','192.168.40.1'],
    ['PC-E',620,460,10,'192.168.110.10','192.168.110.1'],
    ['PC-F',740,460,20,'192.168.120.10','192.168.120.1'],
    ['PC-G',860,460,30,'192.168.130.10','192.168.130.1'],
    ['PC-H',980,460,40,'192.168.140.10','192.168.140.1']
  ];
...
function setFault(kind){
  ...
  if(kind==='L1'){
    const l = state.links.find(l=>{
      ...
      const b=deviceById(l.b).name;
      return [a,b].includes('R1')&&[a,b].includes('R2');
    });
    if(l) l.down=true;
  }
...
$('#faultL1').onclick = ()=>setFault('L1');
$('#faultL2').onclick = ()=>setFault('L2');
$('#faultL3').onclick = ()=>setFault('L3');
$('#faultRoute').onclick = ()=>setFault('ROUTE');
$('#fixAll').onclick = loadSample;
